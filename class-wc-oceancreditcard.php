 ...
 *
 * @class 		WC_Gateway_Oceancreditcard
 * @extends		WC_Payment_Gateway
 * @version		1.4
 * @package		WooCommerce/Classes/Payment
 * @author 		Oceanpayment
 */
class WC_Gateway_Oceancreditcard extends WC_Payment_Gateway {
    ...
    public function __construct() {
        $this->id                 = 'oceancreditcard';
//        $this->icon               = apply_filters('woocommerce_oceancreditcard_icon', plugins_url( 'images/VISA.png', __FILE__ ));
        $this->has_fields         = true;
        $this->method_title       = __( 'Oceanpayment CreditCard', 'oceanpayment-creditcard-gateway' );
        $this->method_description = __( '', 'oceanpayment-creditcard-gateway' );
        $this->supports           = [
            'products',
//            'tokenization',
//            'add_payment_method',
        ...
        $this->Title = $this->get_option('_oceancreditcard_title') ? $this->get_option('_oceancreditcard_title') : '';
        $this->Body = $this->get_option('_oceancreditcard_description') ? $this->get_option('_oceancreditcard_description') : '';

        // Actions
        add_action( 'woocommerce_api_wc_gateway_oceancreditcard', array( $this, 'check_ipn_response' ) );
        add_action( 'woocommerce_update_options_payment_gateways_' . $this->id, array( $this, 'process_admin_options' ) );
        add_action( 'valid-oceancreditcard-standard-itn-request', array( $this, 'successful_request' ) );
        add_action( 'woocommerce_receipt_' . $this->id, array( $this, 'receipt_page' ) );
        add_action( 'woocommerce_thankyou_oceancreditcard', array( $this, 'thankyou_page' ) );
...
//        add_action( 'woocommerce_receipt_oceancreditcard', array( $this, 'oceancreditcard_receipt_page' ) );
    }
            ...
            if($return_info['response_type'] == 1){
                ...
                if(strtoupper($local_signValue) == strtoupper($return_info['signValue'])){
                    ...
                    if ($return_info['payment_status'] == 1) {
                        //成功
                        $order->update_status( 'processing', __( $testorder.$return_info['payment_details'], 'oceanpayment-creditcard-gateway' ) );
                        ...
                        if(empty($this->completed_orders()) || !in_array($return_info['order_number'], $this->completed_orders())){
                    		$order->update_status( 'on-hold', __( $testorder.$return_info['payment_details'], 'oceanpayment-creditcard-gateway' ) );
                		}
                        ...
                        if($errorCode == '20061'){
                        	...
                        	if(empty($this->completed_orders()) || !in_array($return_info['order_number'], $this->completed_orders())){
                    			$order->update_status( 'failed', __( $testorder.$return_info['payment_details'], 'oceanpayment-creditcard-gateway' ) );
                			}
                        }
    ...
    function return_payment( $order ) {
        ...
        if(strtoupper($local_signValue) == strtoupper($back_signValue)){
            ...
            if ($payment_status == 1) {
                //成功
                $order->update_status( 'processing', __( $testorder.$payment_details, 'oceanpayment-creditcard-gateway' ) );
                WC()->cart->empty_cart();
                ...
                if(empty($this->completed_orders()) || !in_array($_REQUEST['order_number'], $this->completed_orders())){
                	$order->update_status( 'on-hold', __( $testorder.$payment_details, 'oceanpayment-creditcard-gateway' ) );
        		}
                $url = $this->get_return_url( $order );
                wc_add_notice( $testorder.$payment_details, 'success' );
                ...
                if($errorCode == '20061'){
					...
					if(empty($this->completed_orders()) || !in_array($_REQUEST['order_number'], $this->completed_orders())){
                		$order->update_status( 'failed', __( $testorder.$payment_details, 'oceanpayment-creditcard-gateway' ) );
        			}
                    $url = esc_url( wc_get_checkout_url() );
